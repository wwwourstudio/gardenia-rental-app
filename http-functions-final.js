...
// ── GET /_functions/adminBookings  — all bookings for admin dashboard
...
// ── POST /_functions/adminSaveItem  — create or update a catalog item
export async function post_adminSaveItem(request) {
  try {
    const body = await request.body.json();
    const isUpdate = !!body._id;
    let result;
    if (isUpdate) {
      result = await wixData.update('RentalCatalog', body, { suppressAuth: true });
    } else {
      result = await wixData.insert('RentalCatalog', body, { suppressAuth: true });
    }
    return ok({ headers: CORS, body: JSON.stringify({ success: true, item: result }) });
  } catch (err) {
    return serverError({ headers: CORS, body: JSON.stringify({ error: err.message }) });
  }
}
export function options_adminSaveItem() { return ok({ headers: CORS, body: '' }); }

// ── POST /_functions/adminUpdateBooking  — update booking status
export async function post_adminUpdateBooking(request) {
  try {
    const { id, status } = await request.body.json();
    if (!id || !status) return badRequest({ headers: CORS, body: JSON.stringify({ error: 'id and status required' }) });
    const existing = await wixData.get('RentalBookings', id, { suppressAuth: true });
    if (!existing) return badRequest({ headers: CORS, body: JSON.stringify({ error: 'Booking not found' }) });
    const updated = await wixData.update('RentalBookings', { ...existing, status }, { suppressAuth: true });
